...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
internal fun VideoCollectionSheet(
    state: VideoPlaybackState,
    viewModel: VideoViewModel,
    onDismiss: () -> Unit
) {
    val season = state.detail?.season ?: return
    val sheetState = rememberModalBottomSheetState(skipPartiallyExpanded = true)
    val currentCid = state.ids.cid
    ...
    ModalBottomSheet(
        ...
        LazyColumn(
            ...
            season.sections.forEachIndexed { sectionIndex, section ->
                item(key = "collection_section_$sectionIndex") {
                ...
                items(
                    items = section.eps,
                    key = { "collection_ep_${it.cid}" }
                ) { episode ->
                    val selected = episode.cid == currentCid
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
                            .clickable(enabled = !selected) {
                                viewModel.switchEpisode(episode.target)
                                onDismiss()
                            }
                        ...
                        Column(
                            ...
                            episode.subTitle?.takeIf(String::isNotBlank)?.let { subTitle ->
                                Text(
                                    text = subTitle,
                                    style = MaterialTheme.typography.bodySmall,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant
...
private fun PlaybackSettingsSection(
    ...
    SheetChoiceCard(
        title = "播完行为",
        subtitle = "合集或分P播放结束后如何处理",
        currentValue = settingsState.playback.playbackEndAction.ordinal,
        options = PlaybackEndAction.entries.indices.toList(),
        label = { PlaybackEndAction.entries[it].label },
        onSelect = { viewModel.updatePlaybackEndAction(PlaybackEndAction.entries[it]) }
    )
