...
class AppSettings @Inject constructor(
    ...
    private val playbackEndActionKey = intPreferencesKey("playback_end_action")
    ...
    val state: Flow<PlayerSettingsState> = context.appSettingsDataStore.data.map { prefs ->
        PlayerSettingsState(
            ...
            playback = PlayerPlaybackPrefs(
                ...
                playbackEndAction = prefs[playbackEndActionKey]
                    ?.let { PlaybackEndAction.entries.getOrNull(it) }
                    ?: defaultPlaybackPrefs.playbackEndAction
            ),
    ...
    suspend fun setPlaybackEndAction(action: PlaybackEndAction) {
        context.appSettingsDataStore.edit { it[playbackEndActionKey] = action.ordinal }
    }
